...
void main() {
  ...
  group('Now Playing Movies', () {
    ...
    test('should return connection failure when the device is not connected to internet',() async {
      ...
      // arrange
      when(mockRemoteDataSource.getNowPlayingMovies()).thenThrow(const SocketException('Failed to connect to the networks'));
  ...
  group('Popular Movies', () {
    ...
    test(
      ...
      // arrange
      when(mockRemoteDataSource.getPopularMovies()).thenThrow(const SocketException('Failed to connect to the networks'));
  ...
  group('Top Rated Movies', () {
    ...
    test(
        'should return ConnectionFailure when device is not connected to the internet',() async {
      // arrange
      when(mockRemoteDataSource.getTopRatedMovies()).thenThrow(const SocketException('Failed to connect to the networks'));

      // act
      final result = await repository.getTopRatedMovies();

      // assert
      expect(result,const Left(ConnectionFailure('Failed to connect to the networks')));
    });
  });

  group('Get Movie Detail', () {
    const tId = 1;
    const tMovieResponse =
      MovieDetailResponse(
        adult: false,
    ...
    test(
        'should return Movie data when the call to remote data source is successful', () async {
      ...
      expect(
        result,
        equals(const Right(testMovieDetail)));
    });
    ...
    test('should return connection failure when the device is not connected to internet',() async {
      ...
      // arrange
      when(mockRemoteDataSource.getMovieDetail(tId)).thenThrow(const SocketException('Failed to connect to the networks'));
      // act
      final result = await repository.getMovieDetail(tId);
      // assert
      verify(mockRemoteDataSource.getMovieDetail(tId));
  ...
  group('Seach Movies', () {
    ...
    test('should return ConnectionFailure when device is not connected to the internet', () async {
      ...
      // arrange
      when(mockRemoteDataSource.searchMovies(tQuery)).thenThrow(const SocketException('Failed to connect to the networks'));
